...
#           The device turns on the relays to power the modem and router display a sign-on on the lcd                                                     #
#           The device will wait for 20 seconds                                                                                                           #
#           The device will try to the WLAN, router and a network device                                                                                  #
#           When it connects, it will give a LCD message and green neopixel                                                                               # 
#           Pings will occur every 30 seconds to ensure connection to WLAN, ROUTER & PIHOLE                                                               #
#           If it loses connection, it will wait 30 seconds to try again twice more                                                                       #
...
# 27-Mar-2025               Version : 1.6     Selectable OTA Updates - 1 minute or 4 hours                                                                #
# 28-Mar-2025               Version : 1.7     Used altneopixel library to stop misbehaving pixels. Used "GRB" mode                                        #
#                                                                                                                                                         #
# Stitching and butchery done by Tony Flynn. Try not to snigger                                                                                           #
#                                                                                                                                                         #
#=========================================================================================================================================================#
PROG_NAME = "Throat Cutter"
CURR_VER  = "Version 1.7"
CURR_DATE = "28-Mar-25"
...
from altneopixel import altNeopixel
...
#            wdt.feed()                                                    # Reset the watchdog timer
        ...
        attempt_conn += 1 
        strip.set_pixel(0,magenta)                                              # Set the color
        strip.show()  
        utime.sleep(led_delay)                                                  # a little breathing space      
...
#            print("WLAN status:", wlan.status())
            strip.set_pixel(0,white)                                            # Set the color
            strip.show()  
            utime.sleep(led_delay)                                              # a little breathing space
    ...
    while True:
        if wlan and wlan.status() == 3:                                     # If connected
            ...
            if WIFI_FLAG == False:
                ...
                display.show()
            WIFI_FLAG = True
            strip.set_pixel(0,green)                                            # Set the color
            strip.show()  
            utime.sleep(led_delay)                                              # a little breathing space
...
#             wdt.feed()                                                        # Reset the watchdog timer
            ...
            strip.set_pixel(0,blue)                                             # Set the color
            strip.show()  
            utime.sleep(led_delay)                                              # a little breathing space
...
# #            wdt.feed()                                                        #Feed the watchdog
            ...
            strip.set_pixel(0,blank)                                            # Set the color
            strip.show()  
            utime.sleep(led_delay)                                              # a little breathing space                              
...
def disconnect_modem():
    ...
    strip.set_pixel(0,red)                                                      # Set the color
    strip.show()  
    utime.sleep(led_delay)                                                      # a little breathing space
...
def disconnect_router():
    ...
    strip.set_pixel(0,yellow)                                                   # Set the color
    strip.show()  
    utime.sleep(led_delay)                                                      # a little breathing space
...
def disconnect_both():
    ...
    RLY_2.off()
    strip.set_pixel(0,cyan)                                                     # Set the color
    strip.show()  
    utime.sleep(led_delay)                                                      # a little breathing space
...
firmware_url   = "https://raw.githubusercontent.com/LookMum-NoHands/Throat-Cutter/main/Code"
...
# wdt = WDT(timeout=8000)                                                       #set for 6 seconds
...
ota_white  = Pin(10, Pin.OUT, value=0)                                          # OTA 2 second indication
...
MIN_OTA    = Pin(17, Pin.IN, Pin.PULL_UP)                                       # Header pin for OTA time
TARGET_OTA = 0                                                                  # Time between OTA events
...
numpix = 1
strip = altNeopixel(numpix, 0, 13, "GRB")                                       # set the number of pixels, state machine, gpio pin and color arragement
strip.brightness(15)                                                            # Note: color arragement can be "RGB", "GRB", "RGBW" or whatever is the physical order
led_delay = 0.004
red       = (255, 0, 0)
orange    = (255, 50, 0)
yellow    = (255, 255, 0)
...
white     = (128,128,128)
blank     = (0,0,0)
# strip.set_pixel(0,red)                                                        # usage for setting a single led. "0" can be replaced by led number, indexed at 0
...
if Wifi_on == False:                                                         # If not connected
    monitor_wifi()
    display.fill(0)
    display.show()
    sleep_ms(250)
    display.fill(0)
    display.text("Attempting",20, 15, 1)
    display.text("Wifi", 40, 30, 1)
    display.text("Connection", 20, 45, 1)                                     
    display.show()
...
if MIN_OTA.value() == True:                                                     # decide is short or long OTA is selected                                                  
    print("4 hour OTA selected")
else:
...
# #   wdt.feed()                                                                    #Feed the watchdog
    ...
    c = c + 1
    display.text(str(c), 62, 5, 1)                                              # Display the counter at top of the LCD
    display.show()
    strip.set_pixel(0,green)                                                    # Set the color
    strip.show()  
    utime.sleep(led_delay)                                                      # a little breathing space
#    np[0] = (0, GREEN_V, 0)                                                     # Set the neopixel LED to green. RGB values
#    np.write()
#    np[1] = (0, GREEN_V, 0)                                                     # Set the neopixel LED to green. RGB values
#    np.write()
    if MIN_OTA.value() == False:                                                # toggle the white led @1Hz id short OTA
        ota_white.toggle()
    sleep_ms(10)
    if c >= interval:
        strip.set_pixel(0,green)                                                # Set the color
...
# Test for OTA activation
        ...
        if MIN_OTA.value() == False:                                            # decide is short or long OTA is selected
            TARGET_OTA = 2
            ota_white.toggle()
        else:
            TARGET_OTA = FULL_OTA
            print("OTA period is ", TARGET_OTA)
            
        if TARGET_OTA == 2 and ota_count == 2: 
            print("1 minute OTA updates")                                       # only print this before version check on fast OTA
            ota_white.toggle()

        if MIN_OTA.value() == True:                                             # set of long OTA
            if ota_count % 5 == 0:                                              # toggle the white led on for 30 seconds every half hour
                ota_white.on()
            else:
                ota_white.off()
                
        print("OTA count is currently ", ota_count, " out of a target of ", TARGET_OTA)
                      
           
        
        if ota_count >= TARGET_OTA:                                             # Every minute, or 120 per hour = 4 hours
            gc.collect()                                                        # never hurts to throw things out
            micropython.mem_info()                                              # print out Pico status
            ota_count = 0        
            ota_updater.download_and_install_update_if_available()              # do the ota check magic here
            print("....Software Check Completed....")
